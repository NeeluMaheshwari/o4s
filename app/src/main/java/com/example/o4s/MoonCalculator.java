...
public class MoonCalculator {
	 ...
	 */
	public MoonCalculator(int year, int month, int day, int h, int m, int s,
						  String obsLon, String obsLat) throws Exception {
	 ...
	 * @throws Exception If the date does not exists.
	 */
	public MoonCalculator(Calendar calendarSunMoon, String obsLon, String obsLat) throws Exception {
	...
	private double[] getMoonDiskOrientationAngles(double lst, double sunRA, double sunDec,
		...
		x = Math.tan(Double.parseDouble(obsLat)) * Math.cos(moonDec) - Math.sin(moonDec) * Math.cos(lst - moonRA);
	...
	private double[] doCalc(double[] pos) {
		...
		double lst = 0;
		try {
			lst = gmst + f.parse(obsLon).doubleValue();
		...
		double correction[] = new double[] {
				radiusAU * Math.cos(Double.parseDouble(obsLat)) * Math.cos(lst),
				radiusAU * Math.cos(Double.parseDouble(obsLat)) * Math.sin(lst),
				radiusAU * Math.sin(Double.parseDouble(obsLat))};
		...
		double sinlat = Math.sin(Double.parseDouble(obsLat));
		double coslat = Math.cos(Double.parseDouble(obsLat));
		...
		tmp = (Math.sin(tmp) - Math.sin(Double.parseDouble(obsLat)) * Math.sin(dec)) / (Math.cos(Double.parseDouble(obsLat)) * Math.cos(dec));
		...
		double transit_alt = Math.asin(Math.sin(dec) * Math.sin(Double.parseDouble(obsLat)) + Math.cos(dec) * Math.cos(Double.parseDouble(obsLat)));
	...
	public static String getDateAsString(double jd) throws Exception {
		...
		int date[] = MoonCalculator.getDate(jd);
